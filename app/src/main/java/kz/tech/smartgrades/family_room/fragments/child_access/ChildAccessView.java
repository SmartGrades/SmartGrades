...
public class ChildAccessView extends RelativeLayout {
    ...
    private static final String[] PARAM_CONTINUE = {"prt:RelLay", "layW:mPrt", "layH:mPrt", "parH:56", "mrg:0,0,10,0",
            "pad:5,5,5,5", "grv:RCV", "txtS:18", "Rule:BOTTOM", "Rule:RIGHT"};
    ...
    public ChildAccessView(Context context, Resources res) {
        super(context);
  ...
  //      onLockAppAccess(linearLayout);
        ...
        /////////////             CONTINUE             /////////////////
        tvContinue = new CustomTextView().onCustom(context, PARAM_CONTINUE, res.getString(R.string.btn_continue));
        this.addView(tvContinue);
        tvContinue.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View view) {
                if (isAccess) {
                    if (onItemClickListener != null) {
                        onItemClickListener.onNextPageClick();
                    }
                } else {
                    if (onItemClickListener != null) {
                        onItemClickListener.onMsgClick(res.getString(R.string.service_killer_permission));
                    }
                }
            }
        });
    ...
    public ChildAccessView(Context context, AttributeSet attrs) {
        super(context, attrs);
    }
    public ChildAccessView(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
    }
    @RequiresApi(api = Build.VERSION_CODES.LOLLIPOP)
    public ChildAccessView(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
