...
//        holder.tvFocusedChild.setText("Имя Фамилия");
        ...
        if (!stringIsNullOrEmpty(m.getAvatar())) Picasso.get().load(m.getAvatar()).fit().centerInside().into(holder.civAvatar);
        else Picasso.get().load(R.drawable.img_default_avatar).fit().centerInside().into(holder.civAvatar);
    }
