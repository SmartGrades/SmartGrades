...
public class ParentSponsorInterFormDialog extends Dialog implements View.OnClickListener {
    ...
    private ModelInterFormList modelInterFormList;
    ...
    public ParentSponsorInterFormDialog(@NonNull ParentActivity activity, ModelInterFormList modelInterFormList) {
        super(activity, R.style.CustomDialog2);
        this.activity = activity;
        this.modelInterFormList = modelInterFormList;
    ...
    private void setData() {
        String avatar = modelInterFormList.getChildAvatar();
        if (!stringIsNullOrEmpty(avatar)) Picasso.get().load(avatar).fit().centerInside().into(civChildAvatar);
        avatar = activity.login.loadUserDate(LoginKey.AVATAR);
        if (!stringIsNullOrEmpty(avatar)) Picasso.get().load(avatar).fit().centerInside().into(civParentAvatar);
        tvChildLogin.setText(modelInterFormList.getChildFirstName() + " " + modelInterFormList.getChildLastName());
        if (!stringIsNullOrEmpty(modelInterFormList.getAverageGrade())) tvAverageGrade.setText(modelInterFormList.getAverageGrade());
        if (!stringIsNullOrEmpty(modelInterFormList.getCountGrade())) tvGradeCount.setText(modelInterFormList.getCountGrade());
        if (!stringIsNullOrEmpty(modelInterFormList.getReward())) tvAmountOfPayments.setText(modelInterFormList.getReward());

        if (!stringIsNullOrEmpty(modelInterFormList.getLessonName1())) {
            cvLesson[0].setVisibility(View.VISIBLE);
            tvSelectLessonLabel[0].setText(modelInterFormList.getLessonName1());
        }
        if (!stringIsNullOrEmpty(modelInterFormList.getLessonName2())) {
            cvLesson[1].setVisibility(View.VISIBLE);
            tvSelectLessonLabel[1].setText(modelInterFormList.getLessonName2());
        }
        if (!stringIsNullOrEmpty(modelInterFormList.getLessonName3())) {
            cvLesson[2].setVisibility(View.VISIBLE);
            tvSelectLessonLabel[2].setText(modelInterFormList.getLessonName3());
        }
        if (!stringIsNullOrEmpty(modelInterFormList.getLessonName4())) {
            cvLesson[3].setVisibility(View.VISIBLE);
            tvSelectLessonLabel[3].setText(modelInterFormList.getLessonName4());
        }
    }
    ...
    private void onAccept() {
        ...
        String SOAP = SoapRequest(func_AcceptInterFormOfSponsorship, jsonData.toString());
