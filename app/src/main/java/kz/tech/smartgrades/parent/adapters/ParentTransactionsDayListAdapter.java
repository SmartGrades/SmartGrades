...
public class ParentTransactionsDayListAdapter extends RecyclerView.Adapter<ParentTransactionsDayListAdapter.ViewHolder> {
    ...
    public ParentTransactionsDayListAdapter(ParentActivity activity) {
    ...
    @Override
    public void onBindViewHolder(@NonNull ParentTransactionsDayListAdapter.ViewHolder holder, int position) {
        ...
        if (!stringIsNullOrEmpty(transactionsList.get(position).getGrade())) {
            int grade = Integer.parseInt(transactionsList.get(position).getGrade());
            if (grade == 0)
        ...
        if(!stringIsNullOrEmpty(avatar)){
            Picasso.get().load(avatar).fit().centerInside().into(holder.civAvatarMentor);
        }
        else{
