package kz.tech.smartgrades.parents_module.contracts.adapter;
...
import kz.tech.smartgrades.parents_module.coins.adapters.StepsAdapter;
...
public class ContractsAdapter extends RecyclerView.Adapter<ContractsAdapter.ViewHolder>{
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        holder.tvDateDoneChild.setText("");
        holder.tvDateDoneParent.setText("");
        holder.tvChildGOOD.setVisibility(View.GONE);
        holder.tvChildGOOD.setText("");
        holder.tvParentGOOD.setVisibility(View.GONE);
        holder.tvParentGOOD.setText("");

        ModelContract modelContract = list.get(position);
        if (modelContract.getIdParent() != null) {
            for (int j = 0; j < familyRoomList.size(); j++) {
                if (familyRoomList.get(j).getIdLogin().equals(modelContract.getIdParent())) {
                    if (familyRoomList.get(j).getAvatar() != null) {
                        Picasso.get().load(familyRoomList.get(j).getAvatar()).fit().centerInside().into(holder.civParent);
                    }
                }
            }
        ...
        if (modelContract.getIdChild() != null) {
            for (int x = 0; x < familyRoomList.size(); x++) {
                if (familyRoomList.get(x).getIdLogin().equals(modelContract.getIdChild())) {
                    if (familyRoomList.get(x).getAvatar() != null) {
                        Picasso.get().load(familyRoomList.get(x).getAvatar()).fit().centerInside().into(holder.civChild);
                    }
                }
            }
        }
        if (modelContract.getAvatar() != null) {
            Picasso.get().load(modelContract.getAvatar()).fit().centerInside().into(holder.civPresentImage);
        }
        ...
        if (modelContract.getCountSteps() != null) {
            ...
            if (total == totalDone) {
                ...
                if (modelContract.getDateChild() != null) {
                    ...
                    holder.tvChildGOOD.setVisibility(View.VISIBLE);
                    holder.tvChildGOOD.setText(res.getString(R.string.fecit));
                }
                if (modelContract.getDateParent() != null) {
                    if (!modelContract.getDateParent().equals("")) {
                        holder.tvDateDoneParent.setText(modelContract.getDateParent());
                        holder.tvParentGOOD.setVisibility(View.VISIBLE);
                        holder.tvParentGOOD.setText(res.getString(R.string.fecit));
                    } else {
...
//Picasso.with(context).load(url).fit().centerInside().into(imageView);
    ...
    class ViewHolder extends RecyclerView.ViewHolder {
        ...
        TextView tvContractTitle, tvWarning, tvDateDoneChild, tvDateDoneParent,
                tvFulfills, tvChildGOOD, tvParentGOOD;
        ...
        ViewHolder(@NonNull View itemView) {
            ...
            tvChildGOOD = (TextView) itemView.findViewById((int)141403);
            tvParentGOOD = (TextView) itemView.findViewById((int)141406);
        }
    }//
    public void setData(List<ModelContract> list, List<TableFamilyRoom> familyRoomList, Resources res) {
        //this.list.clear();
