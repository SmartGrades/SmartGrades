...
public class SignInContractDialog extends Dialog {
    ...
    public SignInContractDialog(Context con, int themeResId, Resources res, ModelContract m, String p, String c) {
        ...
        if (c != null) {
            Picasso.get().load(c).fit().centerInside().into(civAvatarChild);
        }
        ...
        if (p != null) {
            Picasso.get().load(p).fit().centerInside().into(civAvatarParent);
        }
        ...
        if (urlImage != null) {
            Picasso.get().load(urlImage).fit().centerInside().into(ivPresent);
        }
