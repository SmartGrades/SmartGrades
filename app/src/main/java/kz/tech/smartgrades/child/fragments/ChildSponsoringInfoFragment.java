...
import kz.tech.smartgrades.child.adapters.ChildSponsorGradesListAdapter;
import kz.tech.smartgrades.child.adapters.ChildSponsorPeriodListAdapter;
...
public class ChildSponsoringInfoFragment extends Fragment implements View.OnClickListener{

    private ChildActivity activity;
    private ModelChildSponsorship m;
    ...
    public ChildSponsoringInfoFragment(ModelChildSponsorship modelChildSponsorship) {
    ...
    private void setSponsoringData() {
        ...
        if (!listIsNullOrEmpty(m.getLessons())) {
            if (m.getLessons().size() >= 1) {
                ...
                ChildSponsorGradesListAdapter adapter = new ChildSponsorGradesListAdapter(activity);
        ...
        if (!listIsNullOrEmpty(m.getPeriods())) {
            ChildSponsorPeriodListAdapter adapter = new ChildSponsorPeriodListAdapter(activity, m);
