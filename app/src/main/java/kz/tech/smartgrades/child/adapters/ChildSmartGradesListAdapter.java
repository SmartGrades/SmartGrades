...
public class ChildSmartGradesListAdapter extends RecyclerView.Adapter<ChildSmartGradesListAdapter.ViewHolder> {
    ...
    public void onBindViewHolder(@NonNull ChildSmartGradesListAdapter.ViewHolder holder, int position) {
        ...
        if(listIsNullOrEmpty(model.getMentors())) holder.relativeLayout.setVisibility(View.INVISIBLE);
        else {
            if(!stringIsNullOrEmpty(model.getMentors().get(model.getMentors().size()-1).getAvatar())){
                Picasso.get().load(model.getMentors().get(model.getMentors().size()-1).getAvatar()).fit().centerCrop().into(holder.civAvatar);
            }
            if(model.getMentors().size()==1){
                ...
                if(!stringIsNullOrEmpty(model.getMentors().get(model.getMentors().size()-2).getAvatar())){
                    Picasso.get().load(model.getMentors().get(model.getMentors().size()-2).getAvatar()).fit().centerCrop().into(holder.civAvatar2);
                }
            }
            else if (model.getMentors().size() > 2) {
                holder.civAvatar2.setVisibility(View.GONE);
        ...
        if(!stringIsNullOrEmpty(model.getAverageGrade())) holder.tvChildLessonRating.setText(model.getAverageGrade());
        ...
        if (!stringIsNullOrEmpty(model.getSchoolId())) holder.ivSchoolLesson.setVisibility(View.VISIBLE);
