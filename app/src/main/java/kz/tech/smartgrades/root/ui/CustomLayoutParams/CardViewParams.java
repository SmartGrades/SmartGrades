...
public class CardViewParams {
    public CardView.LayoutParams getParams(Context context, String[] paramsList) {
        int cvMatchParent = CardView.LayoutParams.MATCH_PARENT;
        int cvWrapContent = CardView.LayoutParams.WRAP_CONTENT;
        int layout_width = 0;
        int layout_height = 0;
        CardView.LayoutParams params = null;
        for (int i = 0; paramsList.length > i; i++) {
            String[] data = paramsList[i].split(":");//  position 0 = key, 1 = value
            switch (data[0]) {//  KEY
                case "layW":
                    switch (data[1]) {//  VALUE
                        case "mPrt": layout_width = cvMatchParent; break;
                        case "wCnt": layout_width = cvWrapContent; break;
                        case "0": layout_width = 0; break;
                    }
                    break;
                case "layH":
                    switch (data[1]) {//  VALUE
                        case "mPrt": layout_height = cvMatchParent; break;
                        case "wCnt": layout_height = cvWrapContent; break;
                        case "0": layout_height = 0; break;
                    }
                    break;
            }
        }
        params = new CardView.LayoutParams(layout_width, layout_height);
