...
public class MentorColumnsAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> implements View.OnLongClickListener{
    ...
    public void updateData(ModelMentorData mMentorData){
        ...
        if(mMentorData!=null){
            if(!listIsNullOrEmpty(mMentorData.getClassessColumns()))
                mColumns.addAll(mMentorData.getClassessColumns());

            if(!listIsNullOrEmpty(mMentorData.getStudents())){
                Students = new ArrayList<>();
                Students.addAll(mMentorData.getStudents());
            }
            if(!listIsNullOrEmpty(mMentorData.getSchools())){
                Schools = new ArrayList<>();
                Schools.addAll(mMentorData.getSchools());
            }
            if(!listIsNullOrEmpty(mMentorData.getLessons())){
                Lessons = new ArrayList<>();
                Lessons.addAll(mMentorData.getLessons());
            }
        }
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position){
        ...
        ModelSchoolClassesColumn m = mColumns.get(position);
        ...
        MentorClassAdapter adapter = new MentorClassAdapter(activity, flDelete, mColumns, position, Students, Lessons);
        ...
        if(!listIsNullOrEmpty(m.getClasses())){
            ...
            if(m.getClasses().size() >= 6) layoutParams.bottomMargin += DpToPx(activity, 50);
