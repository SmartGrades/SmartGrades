...
public class DialogMentorAddChildToClass extends Dialog implements View.OnClickListener, MentorClassListAdapterV2.OnItemCLickListener{
    ...
    private ModelInterFormOfMentoring mChild;
    ...
    public interface OnItemClickListener{
        void onItemClick(ModelSchoolClass mSelectClass);
    }

    public DialogMentorAddChildToClass(@NonNull MentorActivity activity, ModelMentorData mentorData, ModelInterFormOfMentoring mChild){
...
//        onTernarButton();
    ...
    private void onShowLessonListClick(){
        if(!listIsNullOrEmpty(modelMentorData.getClassessColumns())){
            tvClassListEmptyLabel.setVisibility(View.GONE);
            ArrayList<ModelSchoolClass> classes = new ArrayList<>();
            for(ModelSchoolClassesColumn _column : modelMentorData.getClassessColumns())
                if(!listIsNullOrEmpty(_column.getClasses())) classes.addAll(_column.getClasses());
            classListAdapter.updateData(classes);
            clLabel.setVisibility(View.GONE);
            clSearch.setVisibility(View.VISIBLE);
        }
        else {
            tvClassListEmptyLabel.setText("Список классов пуст. Пожалуйста добавьте класс.");
