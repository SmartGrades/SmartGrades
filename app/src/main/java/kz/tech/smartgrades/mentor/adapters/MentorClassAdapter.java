...
public class MentorClassAdapter extends RecyclerView.Adapter<MentorClassAdapter.ViewHolder> implements View.OnLongClickListener{
    ...
    public MentorClassAdapter(MentorActivity activity, FrameLayout flDelete, ArrayList<ModelSchoolClassesColumn> mColumns, int position,
        ...
        if(AddedItem && PosAddedItem == position) holder.itemView.setVisibility(View.INVISIBLE);
            ...
            if(!listIsNullOrEmpty(Lessons)){
                ...
                for(ModelSchoolLesson lesson : Lessons){
                    if(lesson.getClassId()!=null && lesson.getClassId().equals(m.getId()))
