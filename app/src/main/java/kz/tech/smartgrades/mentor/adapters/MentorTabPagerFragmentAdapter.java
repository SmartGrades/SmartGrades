...
public class MentorTabPagerFragmentAdapter extends FragmentPagerAdapter{
    ...
    public void setData(ArrayList<ModelInterFormOfMentoring> requests){
        if(pageTab1Fragment!=null && !listIsNullOrEmpty(requests))
            pageTab1Fragment.onSetData(requests);
    }
    public void onSetModelMentorData(ModelMentorData mMentorData){
        pageTab1Fragment.onSetModelMentorData(mMentorData);
    }
}