...
public class BSDSelectStudentClass extends BottomSheetDialog implements View.OnClickListener, SchoolAddTeacherClassListAdapterV2.OnItemCLickListener {
    ...
    private ArrayList<ModelStudentProfileClasses> ClassesSelected;

    private SchoolAddTeacherClassListAdapterV2 ClassListAdapter;

    private BSDSelectStudentClass.OnItemClickListener onItemClickListener;
    ...
    public void setOnItemClickListener(BSDSelectStudentClass.OnItemClickListener onItemClickListener) {
        this.onItemClickListener = onItemClickListener;
    }

    public BSDSelectStudentClass(@NonNull SchoolActivity activity, ArrayList<ModelStudentProfileClasses> ClassesList) {
        super(activity);
                ...
                if(!listIsNullOrEmpty(_column.getClasses()))
                    for(ModelSchoolClass _class : _column.getClasses()){
                        if (!listIsNullOrEmpty(ClassesSelected)) {
                            for(ModelStudentProfileClasses _SelectClass : ClassesSelected){
