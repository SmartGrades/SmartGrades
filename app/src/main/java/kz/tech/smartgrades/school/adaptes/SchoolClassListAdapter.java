...
public class SchoolClassListAdapter extends RecyclerView.Adapter<SchoolClassListAdapter.ViewHolder> implements View.OnLongClickListener{
    ...
    public SchoolClassListAdapter(SchoolActivity activity, FrameLayout flDelete, ArrayList<ModelSchoolClassesColumn> mColumns, int position,
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position){
        ...
        if(AddedItem && PosAddedItem == position) holder.itemView.setVisibility(View.INVISIBLE);
            ...
            /*if(!listIsNullOrEmpty(Students)){
                m.setStudents(new ArrayList<>());
                for(ModelSchoolStudent student : Students){
                    if(student.getClassId().equals(m.getId())) m.getStudents().add(student);
                }
            ...
            /*if(!listIsNullOrEmpty(Teachers)){
                m.setTeachers(new ArrayList<>());
                for(ModelSchoolTeacher teacher : Teachers){
                    if(teacher.getClassId().equals(m.getId())) m.getTeachers().add(teacher);
                }
                if(!listIsNullOrEmpty(m.getTeachers())){
                    ...
                    if(!stringIsNullOrEmpty(avatar)) Picasso.get().load(avatar).fit().centerCrop().into(holder.civAvatar1);
                    if(Teachers.size() > 1){
                        holder.civAvatar2.setVisibility(View.VISIBLE);
                        avatar = m.getTeachers().get(1).getAvatar();
                        if(!stringIsNullOrEmpty(avatar)) Picasso.get().load(avatar).fit().centerCrop().into(holder.civAvatar2);
                    }
                    if(Teachers.size() > 2){
            ...
            /*if(!listIsNullOrEmpty(Lessons)){
                ...
                for(ModelSchoolLesson lesson : Lessons){
                    if(lesson.getClassId().equals(m.getId())) m.getLessons().add(lesson);
                }
                if(!listIsNullOrEmpty(m.getLessons())){
                    holder.tvTitleNotLessons.setVisibility(View.GONE);
                }
                else {
                    holder.tvTitleNotLessons.setVisibility(View.VISIBLE);
                }
            }*/
        }
    }
