...
public class SchoolClassMoveDragListener implements View.OnDragListener{
    ...
    public SchoolClassMoveDragListener(){
    }
    public SchoolClassMoveDragListener(FrameLayout flDelete){
        this.flDelete = flDelete;
    }
    public SchoolClassMoveDragListener(SchoolActivity activity, FrameLayout flDelete){
        this.activity = activity;
        this.flDelete = flDelete;
    }
    public SchoolClassMoveDragListener(RecyclerView rvColumns){
        ...
        switch(event.getAction()){
            ...
            case DragEvent.ACTION_DROP:
                if(vTarget.getId() == vMoveLeft || vTarget.getId() == vMoveRight){
                    ...
                    rvSource = (RecyclerView) vSource.getParent();
                    LinearLayout ll = (LinearLayout) rvSource.getParent();
                    ConstraintLayout cl = (ConstraintLayout) ll.getParent();
                    CardView cv = (CardView) cl.getParent();
                    RecyclerView rv = (RecyclerView) cv.getParent();
                    adapterColumns = (SchoolColumnsAdapter) rv.getAdapter();
