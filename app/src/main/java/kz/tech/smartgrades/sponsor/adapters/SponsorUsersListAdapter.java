...
public class SponsorUsersListAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        ...
        if (holder.getItemViewType() == 0) {
            ...
            if (avatar != null && !avatar.isEmpty()) Picasso.get().load(avatar).fit().centerInside().into(((vhChild) holder).civAvatar);
            ...
            String avatar = m.getAvatar();
            if (avatar != null && !avatar.isEmpty()) Picasso.get().load(avatar).fit().centerInside().into(((vhInterForms) holder).civAvatar);
